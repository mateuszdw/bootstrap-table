	...
	*/
	  ...
	  createControls: function createControls(that, header) {
	    ...
	    $.each(that.columns, function (i, column) {
	      ...
	      if (!column.filterControl && !that.options.filterControlContainer) {
	        ...
	        $.each($filterControls, function (i, filterControl) {
	    ...
	    if (addedFilterControl) {
	      ...
	      UtilsFilterControl.getControlContainer(that).off('mouseup', 'input:not([type=radio])').on('mouseup', 'input:not([type=radio])', function (_ref8) {
	      ...
	      UtilsFilterControl.getControlContainer(that).off('change', 'input[type=radio]').on('change', 'input[type=radio]', function (_ref9) {
	      ...
	      if (UtilsFilterControl.getControlContainer(that).find('.date-filter-control').length > 0) {
	        $.each(that.columns, function (i, _ref10) {
	          var filterControl = _ref10.filterControl,
	              field = _ref10.field,
	              filterDatepickerOptions = _ref10.filterDatepickerOptions;

	          if (filterControl !== undefined && filterControl.toLowerCase() === 'datepicker') {
	            UtilsFilterControl.getControlContainer(that).find(".date-filter-control.bootstrap-table-filter-control-".concat(field)).datepicker(filterDatepickerOptions).on('changeDate', function (_ref11) {
	              var currentTarget = _ref11.currentTarget,
	                  keyCode = _ref11.keyCode;
	              clearTimeout(currentTarget.timeoutId || 0);
	              currentTarget.timeoutId = setTimeout(function () {
	                that.onColumnSearch({
	                  currentTarget: currentTarget,
	...
	$.extend($.fn.bootstrapTable.defaults, {
	  ...
	  filterTemplate: {
	    ...
	    select: function select(_ref12, field) {
	...
	$.BootstrapTable = /*#__PURE__*/function (_$$BootstrapTable) {
	  ...
	  _createClass(_class, [{
	    ...
	    key: "onColumnSearch",
	    value: function onColumnSearch(_ref13) {
	      var currentTarget = _ref13.currentTarget,
	          keyCode = _ref13.keyCode;
	    ...
	    value: function toggleFilterControl() {
	      ...
	      var $filterControls = UtilsFilterControl.getControlContainer(this).find('.filter-control, .no-filter-control');
	      ...
	      this.$toolbar.find('>.columns').find('.filter-control-switch').html(Utils.sprintf(this.constants.html.icon, this.options.iconsPrefix, icon) + ' ' + text);
	    }
	  }]);
